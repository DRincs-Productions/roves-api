...
async function callSteam<T>(command: string, params?: Record<string, unknown>): Promise<T> {
    ...
    const response = await fetch(`steam:${command}${queryString ? `?${queryString}` : ""}`);
...
export const steam: SteamApi = {
    ...
    async unlockAchievement(achievementId) {
        return callSteam<boolean>("unlock_achievement", { achievement_id: achievementId }).catch(() => false);
    },

    async isAchievementUnlocked(achievementId) {
        return callSteam<boolean>("is_achievement_unlocked", { achievement_id: achievementId }).catch(
            () => false,
        );
    },

    async clearAchievement(achievementId) {
        return callSteam<boolean>("clear_achievement", { achievement_id: achievementId }).catch(() => false);
    },

    async setStatInt(name, value) {
        return callSteam<boolean>("set_stat_int", { name, value: Math.trunc(value) }).catch(() => false);
    },
    ...
    async setStatFloat(name, value) {
        return callSteam<boolean>("set_stat_float", { name, value }).catch(() => false);
    },
    ...
    async isDlcInstalled(appId) {
        return callSteam<boolean>("is_dlc_installed", { app_id: appId }).catch(() => false);
    },
    ...
    async openStore(appId) {
        return callSteam<boolean>("open_store", { app_id: appId }).catch(() => false);
    },
};
